 ...
 * @returns ValidationFunctionResult containing overall status and detailed results for each codeblock
 */
export default function validateTypescript(
  ...
  // Overall validation is successful only if all individual checks passed
  const valid = detailedChecks.every(
    (check) => check.result === ValidationResult.SUCCESS,
  );

  return { valid, detailedChecks };
}
