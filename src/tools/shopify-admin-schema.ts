...
async function loadGraphQLSchema() {
  try {
    ...
    if (schemaJson.data && schemaJson.data.__schema) {
      console.error(
        `[shopify-admin-schema-tool] Query type name from schema: ${schemaJson.data.__schema.queryType?.name}`,
      );
      ...
      try {
        ...
        console.error(
          `[shopify-admin-schema-tool] Error building schema: ${buildError}`,
        );
        // Log the first 500 characters of the SDL for debugging
        console.error(
          `[shopify-admin-schema-tool] SDL snippet: ${sdl.substring(0, 500)}...`,
        );
        throw buildError;
      }
    }
    ...
    console.error(
      `[shopify-admin-schema-tool] Error loading GraphQL schema: ${error}`,
    );
    throw error;
  }
}
...
function introspectionToSDL(introspectionData: any): string {
  ...
  const customScalars = introspectionData.__schema.types
    .filter(
      (type: any) => type.kind === "SCALAR" && !isBuiltInScalar(type.name),
    )
      ...
      switch (type.kind) {
          ...
          if (type.fields && type.fields.length > 0) {
            for (const field of type.fields) {
              ...
              if (field.args && field.args.length > 0) {
                sdl += "(";
                sdl += field.args
                  .map((arg: any) => `${arg.name}: ${formatType(arg.type)}`)
                  .join(", ");
                sdl += ")";
              }
        ...
        case "UNION":
          if (type.possibleTypes && type.possibleTypes.length > 0) {
            ...
            console.error(
              `[shopify-admin-schema-tool] Skipping union type ${type.name} with no possible types`,
            );
          }
          break;
      }
  ...
  console.error(
    `[shopify-admin-schema-tool] Generated SDL with query root type: ${queryTypeName}`,
  );
  return sdl;
}
 ...
 */
export async function validateShopifyAdminGraphQLQuery(query: string) {
  try {
    console.error(`[shopify-admin-validate] Validating GraphQL query`);
    console.error(
      `[shopify-admin-validate] Query (truncated): ${query.substring(0, 100)}${query.length > 100 ? "..." : ""}`,
    );

    const result = await validateShopifyAdminQuery(query);
...
async function validateShopifyAdminQuery(queryString: string): Promise<{
  success: boolean;
  ...
  try {
    ...
    try {
      ...
      const significantErrors = validationErrors;
