...
import validateLiquidCodeblocks from "../validations/liquidCodeBlock.js";
...
export async function shopifyTools(server: McpServer): Promise<void> {
  ...
  server.tool(
    "validate_theme",
    `This tool MUST run if the user asks the LLM to create or modify Liquid code inside the Theme repository.

    A theme repository is a directory that contains the following directories:
      - snippets
      - sections
      - blocks
      - config
    ...
    It takes two arguments: absoluteThemePath and conversationId. The absoluteThemePath argument is the path to the theme directory. The conversationId is REQUIRED and must be the conversationId returned from the learn_shopify_apis tool.
    It returns a comprehensive validation result with details for each file in the theme explaining why the content was valid, invalid, or skipped. This detail is provided so LLMs know how to modify file to remove errors within the file.`,

    {
      absoluteThemePath: z.string().describe("The path to the theme directory"),
      conversationId: z
        .string()
        .describe(
          "🔗 REQUIRED: conversationId from learn_shopify_apis tool. Call learn_shopify_apis first if you don't have this.",
    ...
    async ({ absoluteThemePath, conversationId }) => {
      const validationResult = await validateTheme(absoluteThemePath);
      const responseText = validationSummary(validationResult);

      recordUsage(
        "validate_theme",
        "validating theme directory",
        responseText,
        conversationId,
      ).catch(() => {});
  ...
  server.tool(
    "validate_liquid_codeblocks",
    `This tool is used to ensure that the codeblock and supporting files generated by LLMs don't have hallucinated fields or operations. If a user asks for an LLM to generate liquid code, this tool should always be used to ensure valid code blocks and supporting files were generated.

    ⚠️  REQUIRES CONVERSATION ID: You must call learn_shopify_apis first to get a conversationId.

    It takes two arguments: codeblocks and conversationId. The codeblocks argument is an array of objects containing the markdown codeblock, the filename, and the filetype. The conversationId is REQUIRED and must be the conversationId returned from the learn_shopify_apis tool.
    It returns a comprehensive validation result with details for each code block explaining why a code block was valid, invalid, or skipped. This detail is provided so LLMs know how to modify codeblocks to remove errors within generated codeblocks.`,
        ...
        .array(
          z.object({
            fileName: z
              .string()
              .describe(
                "The filename of the codeblock. The filename should be descriptive of the codeblock's purpose, and should be in dashcase.",
              ),
            fileType: z
              .enum([
                "blocks",
              ...
              .default("blocks")
              .describe("The type of codeblock generated"),
            fileExtension: z
              .string()
              .describe("The file extension of the codeblock"),
            content: z.string(),
          }),
        )
        .describe(
          "Array of objects containing the markdown codeblock, the filename, and the filetype",
        ),
      conversationId: z
        .string()
        .describe(
    ...
    async ({ codeblocks, conversationId }) => {
      const validationResult = await validateLiquidCodeblocks(codeblocks);
      const responseText = validationSummary(validationResult);
  ...
  // TODO: We will remove this once we are actually fetching the liquid rules from the dev docs
  gettingStartedApis.push({
    name: "liquid",
    description:
      ...
      "Keywords: liquid, theme, liquid-component, liquid-block, liquid-code, liquid-snippet.",
  });
      ...
      try {
        ...
        if (api === "liquid") {
          try {
            ...
            text = readFileSync(
              join(__dirname, "..", "data", "liquid-rules.txt"),
              "utf8",
            );
          } catch (error) {
            console.error(`Error reading liquid rules file: ${error}`);
            text = `Error reading liquid rules file: ${error instanceof Error ? error.message : String(error)}`;
          }
        } else {
