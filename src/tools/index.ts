...
import { validatePolarisWebComponents } from "../validations/polarisWebComponents.js";
import {
  ValidationResponse,
  ValidationResult,
...
${gettingStartedApis.map((api) => `    - ${api.name}: ${api.description}`).join("\n")}
  ...
  server.tool(
    "validate_polaris_wc_codeblocks",
    `This tool is used to ensure that the codeblock generated by LLMs don't have hallucinated components, props, and/or prop values. If a user asks for an LLM to generate polaris web components, this tool should always be used to ensure valid code blocks were generated.
    It takes one argument: codeblocks, which is an array of markdown code blocks containing HTML with custom elements.
    ...
    async ({ codeblocks }) => {
      ...
      const validationResponses = await Promise.all(
        codeblocks.map(async (block) => {
          return await validatePolarisWebComponents(block);
        }),
      );
