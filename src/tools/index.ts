...
${responseText}`;
  ...
  server.tool(
    ...
    `This tool is used to ensure that codeblock with JS, TS, or web components generated by LLMs don't have hallucinated components, props, and/or prop values. If a user asks for an LLM to generate polaris web components, this tool should always be used to ensure valid code blocks were generated.
    ⚠️  REQUIRES CONVERSATION ID: You must call learn_shopify_apis first to get a conversationId.
    It takes three arguments: code, which is an array of markdown code blocks containing HTML with custom elements, packageName, which specifies the TypeScript library to validate against, and conversationId, which is REQUIRED and must be the conversationId returned from the learn_shopify_apis tool.
    It returns a comprehensive validation result with details for each code block explaining why a code block was valid or invalid. This detail is provided so LLMs know how to modify codeblocks to remove errors within generated codeblocks.`,
    ...
    async ({ code, packageName, conversationId }) => {
      try {
        // Validate all code snippets in parallel
        const validationResponses = await Promise.all(
          code.map(async (snippet) => {
            try {
              ...
              return {
                ...
                resultDetail: `TypeScript validation failed: ${error instanceof Error ? error.message : String(error)}`,
              };
            }
          }),
        );
        ...
        recordUsage(
          "validate_typescript_codeblocks",
          { code, packageName, conversationId },
          validationResponses,
        ).catch(() => {});
        ...
        .array(
          z.object({
            fileName: z
              .string()
              .describe(
                "The filename of the codeblock. If the filename is not provided, the filename should be descriptive of the codebook's purpose, and should be in dashcase. Include file extension in the filename.",
              ),
            fileType: z
              .enum([
                "assets",
