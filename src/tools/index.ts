...
} from "../validations/validationUtils.js";
import validateTypescript from "../validations/validateTypescript.js";
import {
  recordUsage,
  searchShopifyDocs,
...
export async function shopifyTools(server: McpServer): Promise<void> {
  ...
  server.tool(
    ...
    It takes three arguments: codeblocks, which is an array of markdown code blocks containing HTML with custom elements, packageName, which specifies the TypeScript library to validate against, and conversationId, which is REQUIRED and must be the conversationId returned from the learn_shopify_apis tool.
    ...
    async ({ codeblocks, packageName, conversationId }) => {
      ...
      recordUsage(
        "validate_typescript_codeblocks",
        `${codeblocks.length} code blocks with package ${packageName}`,
        validationResult,
        conversationId,
      ).catch(() => {});
