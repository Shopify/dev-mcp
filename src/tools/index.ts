...
import validateLiquidCodeblocks from "../validations/liquid.js";
...
export async function shopifyTools(server: McpServer): Promise<void> {
  ...
  server.tool(
    ...
    `This tool is used to ensure that the codeblock generated by LLMs don't have hallucinated fields or operations. If a user asks for an LLM to generate liquid code, this tool should always be used to ensure valid code blocks were generated.

    ⚠️  REQUIRES CONVERSATION ID: You must call learn_shopify_apis first to get a conversationId.

    It takes two arguments: codeblocks and conversationId. The codeblocks argument is an array of markdown code blocks containing liquid code to validate. The conversationId is REQUIRED and must be the conversationId returned from the learn_shopify_apis tool.
    It returns a comprehensive validation result with details for each code block explaining why a code block was valid, invalid, or skipped. This detail is provided so LLMs know how to modify codeblocks to remove errors within generated codeblocks.`,

    {
      codeblocks: z
        .array(z.string())
        .describe(
          "Array of markdown code blocks containing liquid code to validate",
        ),
      conversationId: z
        .string()
        .describe(
    ...
    async ({ codeblocks, conversationId }) => {
      ...
      const formattedCodeblocks = codeblocks.map((block) => {
        ...
        // Otherwise, wrap in liquid markdown block
        return `\`\`\`liquid\n${block}\n\`\`\``;
      });

      const validationResult = await validateLiquidCodeblocks(
        formattedCodeblocks.join("\n\n"),
      );
      const responseText = validationSummary(validationResult);
